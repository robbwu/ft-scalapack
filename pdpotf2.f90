...
!                                                                       
      IF( INFO.NE.0 ) THEN 
         CALL PXERBLA( ICTXT, 'PDPOTF2', -INFO ) 
...
!                                                                       
                  IF( J.LT.JA+N-1 ) THEN 
                     ICURR = IDIAG + LDA 
                     CALL DGEMV( 'Transpose', J-JA, JA+N-J-1, -ONE,     &
     &                           A( IOFFA+LDA ), LDA, A( IOFFA ), 1,    &
     &                           ONE, A( ICURR ), LDA )                 
                     CALL DSCAL( N-J+JA-1, ONE / AJJ, A( ICURR ), LDA ) 
                  END IF 
                  IDIAG = IDIAG + LDA + 1 
                  IOFFA = IOFFA + LDA 
   10          CONTINUE 
...
!                                                                       
                  IF( J.LT.JA+N-1 ) THEN 
                     ICURR = IDIAG + 1 
                     CALL DGEMV( 'No transpose', JA+N-J-1, J-JA, -ONE,  &
     &                           A( IOFFA+1 ), LDA, A( IOFFA ), LDA,    &
     &                           ONE, A( ICURR ), 1 )                   
                     CALL DSCAL( JA+N-J-1, ONE / AJJ, A( ICURR ), 1 ) 
                  END IF 
                  IDIAG = IDIAG + LDA + 1 
                  IOFFA = IOFFA + 1 
   30          CONTINUE 
